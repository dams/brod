...
handle_reset_offset(#state{offset_reset_policy = Policy} = State, _Error) ->
  ...
  %% there is a chance of crashing here of socket down,
  %% or error when resolving beging offsets etc.
  %% self() will crash and restarted by supervisor
  %% a subscribe call from subscriber is required
